...
Theorem app_nil_r : forall (X:Type), forall l:list X,
  l ++ [] = l.
Proof.
  intros X l. induction l as [|h t H].
    - simpl. reflexivity.
...
Theorem app_assoc : forall A (l m n:list A),
  l ++ m ++ n = (l ++ m) ++ n.
Proof.
  intros A l m n. induction l as [|h t H].
    - simpl. reflexivity.
    - simpl. rewrite H. reflexivity.
Qed.
...
Definition partition {X : Type}
  ...
  let negtest := fun n => negb (test n) in
    (filter test l, filter negtest l).
...
Proof.  
  intros X Y f l1 l2. induction l1 as [|h t H].
    - simpl. reflexivity.
    - simpl. rewrite H. reflexivity.
Qed.
...
Proof.
  intros X Y f l. induction l as [|h t H].
    - simpl. reflexivity.
    - simpl. rewrite map_distr. rewrite H. reflexivity.
Qed.
(** [] *)
...
Lemma fold_length_S: forall X (n: X) (l: list X),
  fold_length (n::l) = S (fold_length l).
Proof. 
  intros X n l. destruct l as [|h t].
    - simpl. reflexivity.
    - simpl. reflexivity.
Qed.
...
Proof.
  intros X l. induction l as [|h t H].
    - simpl. reflexivity.
    - simpl. rewrite fold_length_S. rewrite H. reflexivity. 
Qed.
(** [] *)
...
Lemma fold_map_h_distr: forall X Y (f: X -> Y) (h: X) (t: list X),
  fold_map f (h::t) = [f h] ++ fold_map f t.
Proof.
  intros X Y f h t. simpl. reflexivity.
Qed. 
...
Proof.
  intros X Y f l. induction l as [|h t H].
    - simpl. reflexivity.
    - simpl.  rewrite fold_map_h_distr. rewrite H. simpl. reflexivity. 
Qed.
...
Definition mult (n m : cnat) : cnat := 
  fun (X: Type) (f: X -> X) (x: X) => let mf := m X f in n X mf x.
...
Example exp_4 : exp zero three = zero.
Proof. reflexivity. Qed.
