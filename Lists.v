...
Fixpoint subset (s1 : bag) (s2 : bag) : bool :=
  ...
  | nil, _ => true
  | _, nil => false
  | h::t, _ => if (member h s2) then (subset t (remove_one h s2)) else false
  end.
...
Theorem add_inc_count : forall (s: bag) (n: nat),
  length (add n s) = (length s) + 1.
Proof.
  intros s n. destruct s as [|s'].
    - simpl. reflexivity.
    - simpl. rewrite(add_comm (length s) 1). reflexivity.
Qed.
...
Proof.
  intros l1 l2. induction l1 as [|h t Hl].
    - simpl. rewrite app_nil_r. reflexivity.
    - simpl. rewrite Hl. rewrite app_assoc. reflexivity.
Qed.
...
Proof.
  intros l. induction l as [|h t Hl].
    - simpl. reflexivity.
    - simpl. rewrite rev_app_distr. rewrite Hl. reflexivity.
Qed.
...
Proof.
  intros l1 l2. induction l1 as [|h1 t1 Hl1].
    - simpl. reflexivity.
    - simpl. destruct h1 as [|h1'] eqn:H.
      + rewrite Hl1. reflexivity.
      + rewrite Hl1. simpl. reflexivity.
Qed.
(** [] *)
...
Proof.
  intros l. induction l as [|h t Hl].
    - simpl. reflexivity.
    - simpl. rewrite Hl. rewrite eqb_refl. reflexivity. 
Qed.
(** [] *)
...
Proof.
  intros s. induction s as [|h t Hs].
    - simpl. reflexivity.
    - simpl. destruct h as [|h'].
      + simpl. apply leb_n_Sn. (* apply H <=> rewrite H. reflexivity.*)
      + simpl. apply Hs.
Qed.
(** [] *)
...
Theorem count_sum_dist: forall (b1 b2: bag) (n:nat),
  count n (b1 ++ b2) = count n b1 + count n b2.
Proof.
  intros b1 b2 n. induction b1 as [|h t Hb].
    - simpl. reflexivity.
    - simpl. destruct (h =? n).
...
Theorem rev_injective : forall (l1 l2 : natlist),
  rev l1 = rev l2 -> l1 = l2.
Proof.
  intros l1 l2. destruct l1 as [|h t].
    - intros H. rewrite <- rev_involutive. rewrite <- H. simpl. reflexivity.
    - intros H. rewrite <- rev_involutive. rewrite <- H. rewrite rev_involutive. reflexivity.
Qed.
(** [] *)
...
Proof.
  intros l default. simpl. destruct l as [|h t].
    - simpl. reflexivity.
    - simpl. reflexivity. 
Qed.
(** [] *)
...
Proof.
  intros x. destruct x as [n]. simpl. apply eqb_refl.
Qed.
(** [] *)
...
(* 0 because no baz element can be constructed *)
